...
class TestEvictionVersion0Compat:
    """Loading VERSION=0 files (no stats) assigns zero stats to all entries."""

    def _write_v0_file(self, path, keys, D=4, opt_type="adam", lr=0.01,
                       beta1=0.9, beta2=0.999, eps=1e-8, t=0, bs=1_000_000):
        """Write a minimal VERSION=0 binary file for testing."""
        import struct as _st

        # For V0: each entry stores key+slot+weight+grad+[m+v]
        n = len(keys)
        entry_bytes = n * (8 + 4 + D * 4 + D * 4 + D * 4 + D * 4)

        with open(path, "wb") as f:
            # Header
            f.write(b"HASHEMB\x00")
            f.write(_st.pack("<i", 0))           # version = 0
            f.write(_st.pack("<q", n))            # n_total
            f.write(_st.pack("<i", D))            # D
            opt_buf = opt_type.encode().ljust(8, b"\x00")[:8]
            f.write(opt_buf)
            f.write(_st.pack("<f", lr))
            f.write(_st.pack("<f", beta1))
            f.write(_st.pack("<f", beta2))
            f.write(_st.pack("<f", eps))
            f.write(_st.pack("<q", t))
            f.write(_st.pack("<q", bs))

            # 16 buckets: only bucket 0 has entries
            for b in range(16):
                nb = n if b == 0 else 0
                f.write(_st.pack("<q", nb))
                f.write(_st.pack("<i", b))

                if b == 0:
                    for i, k in enumerate(keys):
                        slot = i  # V0 stores explicit slot
                        f.write(_st.pack("<q", k))
                        f.write(_st.pack("<i", slot))
                        f.write(b"\x00" * (D * 4))  # weight (zeros)
                        f.write(b"\x00" * (D * 4))  # grad (zeros)
                        f.write(b"\x00" * (D * 4))  # m (zeros)
                        f.write(b"\x00" * (D * 4))  # v (zeros)

    def test_v0_load_assigns_zero_stats(self):
        """VERSION=0 entries get update_count=0, last_step=0."""
        with tempfile.NamedTemporaryFile(suffix=".hashemb", delete=False) as f:
            fpath = f.name
        try:
            self._write_v0_file(fpath, [10, 20, 30])

            table = _hashemb_cpp.HashEmbeddingTable(100, 4, optimizer="adam", lr=0.01)
            table.load(fpath)
            assert table.num_entries == 3

            # min_count=1 should evict all V0 entries (count=0 < 1).
            new_t = _roundtrip(table, min_count=1)
            assert new_t.num_entries == 0
        finally:
            os.unlink(fpath)

    def test_v0_load_then_train_stats_work(self):
        """After loading V0, training creates stats that eviction can use."""
        with tempfile.NamedTemporaryFile(suffix=".hashemb", delete=False) as f:
            fpath = f.name
        try:
            self._write_v0_file(fpath, [10, 20])

            table = _hashemb_cpp.HashEmbeddingTable(100, 4, optimizer="adam", lr=0.01)
            table.load(fpath)

            # Train key 10 for 5 steps, leave key 20 untouched.
            for _ in range(5):
                keys = np.array([10], dtype=np.int64)
                _, slots = table.lookup_and_gather(keys)
                table.scatter_add_grad(slots, np.ones((1, 4), dtype=np.float32))
                table.step()

            # Key 10: count=5, idle=0 → kept by min_count=5.
            # Key 20: count=0 (from V0), idle=5 → evicted by min_count=5.
            new_t = _roundtrip(table, min_count=5)
            assert new_t.num_entries == 1
        finally:
            os.unlink(fpath)
